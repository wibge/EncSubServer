...
def get_identity_url(request):
    user = Auth.AuthenticatedUser(request)
    if not user:
      return None
    ...
    return request_url_without_path+'/id/'+user.nickname()
...
def InitializeOpenId():
  ...
  if port == "443":
    protocol = "https"
  op_endpoint = "%s://%s%s/id/server"%(protocol,name,":%s"%port if port!="80" else "") if name else None
  logging.info('op_endpoint: %s',op_endpoint)
  logging.info('openid version: %s', openid.__version__)
...
#    return True
    ...
    if path[4:] != user.nickname():
      expected = parsed[0] + '://' + parsed[1] + '/id/' + user.nickname()
...
class Login(Handler):
  ...
  def get(self):
    ...
    login_url = login_url = '/auth/loginForm?redirect=%s' % redirect_url
    user = Auth.AuthenticatedUser(self.request)
    if user:
...
#mrk
    ...
    if args.has_key('yes'):
      ...
      if args.get('remember', '') == 'yes':
        logging.info('Setting cookie to remember openid login for 1 year')
